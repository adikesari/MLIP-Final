...
class BaseModel():
    """Base model for deblurring task."""
    ...
    def load_network(self, net, name, tag='net_sr'):
        ...
        if self.opt.get('test_only'):
            ...
            if tag == 'net_deblur':
                load_path = osp.join(self.exp_dir, 'models/net_deblur_latest.pth')
    ...
    def resume_training(self, resume_path):
        ...
        if hasattr(self, 'optimizer_deblur') and 'optimizer_deblur' in resume_state.keys():
            self.optimizer_deblur.load_state_dict(resume_state['optimizer_deblur'])
        ...
        if hasattr(self, 'net_deblur') and 'net_deblur' in resume_state.keys():
            self.get_bare_model(self.net_deblur).load_state_dict(resume_state['net_deblur'], strict=True)
    ...
    def visualize(self, img, output, filename, imwrite=True, lr_img=None):
        ...
        deblur_dir = osp.join(self.exp_dir, 'visualize', 'deblurred_images')
        ...
        if imwrite:
            ...
            cv2.imwrite(osp.join(deblur_dir, f'{filename}_deblurred.png'), cv2.cvtColor(output, cv2.COLOR_RGB2BGR))
    ...
    def calculate_cost(self):
        ...
        inp_shape = (3, 480, 480)
        macs_deblur, params_deblur = get_model_complexity_info(self.net_deblur, inp_shape, verbose=False, print_per_layer_stat=False)
        print('deblur network :')
        print('{:<30}  {:<8}'.format('Computational complexity: ', macs_deblur))
        print('{:<30}  {:<8}'.format('Number of parameters: ', params_deblur))
        return 